//! Demonstrates `AnimDespawnOnExit`: spawn a sprite in `StateA`, switch to
//! `StateB` and watch it scale to zero before being despawned automatically.
//!
//! Run from the workspace root:
//!
//!   cargo run --example despawn_on_exit -p anim_ui
//!
//! Controls:
//!   SPACE - toggle between StateA and StateB
//!   ESC   - exit

use anim_ui::{AnimDespawnOnExit, AnimSpawnOn, AnimUiPlugin};
use bevy::prelude::*;
...
fn main() {
    ...
    app.add_plugins(DefaultPlugins.set(WindowPlugin {
        primary_window: Some(Window {
            title: "anim_ui – despawn_on_exit".to_string(),
...
fn spawn_sprite(mut commands: Commands, state: Res<State<DemoState>>) {
    ...
    commands.spawn((
        ...
        // Declare ownership: scale-to-zero + despawn fires when state changes.
        AnimSpawnOn,
        AnimDespawnOnExit(state.get().clone()),
    ));
}
