...
fn fragment(mesh: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // Scanline darkening contribution to alpha
    let frag_y = mesh.position.y;
    var scanline = 1.0;
    if (i32(floor(frag_y * 0.25)) % 2 == 0) {
        scanline = 1.0 - 0.15 * noise;
    }

    let out_alpha = noise * scanline * settings.alpha;

    return vec4<f32>(1.0, 1.0, 1.0, out_alpha);
}
