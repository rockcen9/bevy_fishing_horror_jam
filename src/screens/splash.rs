...
pub(super) fn plugin(app: &mut App) {
    ...
    app.add_systems(
        Update,
        (
            tick_splash_fade_animation.in_set(PostPhysicsAppSystems::TickTimers),
            apply_splash_image_alpha.in_set(PostPhysicsAppSystems::Update),
        )
    ...
    app.add_systems(
        ...
        (
            ...
            advance_to_gameplay_on_splash_done.in_set(PostPhysicsAppSystems::Update),
        )
            .run_if(in_state(Screen::Splash)),
    );
    ...
    commands
        .spawn((
            ...
            children![(
                ...
                ImageNode::new(asset_server.load("embedded://bevy_game/screens/files/splash.png")),
                SplashFadeAnimation {
...
fn tick_splash_fade_animation(time: Res<Time>, mut animation_query: Query<&mut SplashFadeAnimation>) {
    for mut anim in &mut animation_query {
        anim.t += time.delta_secs();
    }
}

fn apply_splash_image_alpha(mut animation_query: Query<(&SplashFadeAnimation, &mut ImageNode)>) {
...
fn advance_to_gameplay_on_splash_done(timer: ResMut<SplashTimer>, mut next_screen: ResMut<NextState<Screen>>) {
    if timer.0.just_finished() {
        next_screen.set(Screen::Loading);
    }
}

fn advance_to_gameplay_on_escape(mut next_screen: ResMut<NextState<Screen>>) {
    next_screen.set(Screen::Loading);
}
